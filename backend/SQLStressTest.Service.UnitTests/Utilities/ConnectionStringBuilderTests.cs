...
public class ConnectionStringBuilderTests : TestBase
{
    ...
    [Fact]
    public void BuildForExtendedEvents_DoesNotIncludeTrustServerCertificate()
    {
        // Arrange
        var config = CreateTestConnectionConfig();

        // Act
        var connectionString = _builder.BuildForExtendedEvents(config);

        // Assert
        Assert.DoesNotContain("TrustServerCertificate", connectionString);
        Assert.DoesNotContain("Trust Server Certificate", connectionString);
    }

    [Fact]
    public void BuildForExtendedEvents_IncludesServer()
    {
        // Arrange
        var config = CreateTestConnectionConfig(server: "testserver");
    ...
    [Fact]
    public void BuildForExtendedEvents_IncludesDatabase()
    {
        // Arrange
        var config = CreateTestConnectionConfig(database: "testdb");
    ...
    [Fact]
    public void BuildForExtendedEvents_IncludesIntegratedSecurity_WhenTrue()
    {
        // Arrange
        var config = CreateTestConnectionConfig(integratedSecurity: true);
    ...
    [Fact]
    public void BuildForExtendedEvents_IncludesUsernameAndPassword_WhenIntegratedSecurityIsFalse()
    {
        // Arrange
        var config = CreateTestConnectionConfig(integratedSecurity: false);
        config.Username = "testuser";
